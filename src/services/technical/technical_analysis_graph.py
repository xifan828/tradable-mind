...
from src.utils.llm import invoke_gemini_model
...
def create_single_technical_analysis(state: SingleTechnicalAnalysisState):
    ...
    ai_msg = invoke_gemini_model(
        ...
        model_type="2.5_flash",
        include_paid_key=False,
    )
    if isinstance(ai_msg.content, list) and ai_msg.content and isinstance(ai_msg.content[0], dict):
        answer = ai_msg.content[0].get("text", "")
    else:
